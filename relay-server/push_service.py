...
class PushService:
    ...
    def __init__(self, private_key_path: str = None, subject: str = None):
        ...
        self.subject = subject or os.getenv(
            "ZHINENG_BRIDGE_VAPID_SUBJECT", "mailto:admin@zhineng-bridge.com"
        )
    ...
    def _load_vapid_keys(self) -> tuple:
        ...
        if not os.path.exists(self.private_key_path):
            raise FileNotFoundError(f"VAPID private key file not found: {self.private_key_path}")
    ...
    async def subscribe(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if not endpoint:
                return web.json_response(
                    {"type": "error", "message": "Missing 'endpoint' in subscription", "code": 400},
                    status=400,
                )
            ...
            self.logger.info(
                "Push subscription registered", subscription_id=subscription_id, endpoint=endpoint
            )
            ...
            return web.json_response(
                {"type": "error", "message": "Invalid JSON in request body", "code": 400},
                status=400,
            )
    ...
    async def unsubscribe(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if not subscription_id:
                return web.json_response(
                    {"type": "error", "message": "Subscription not found", "code": 404}, status=404
                )
            ...
            if subscription_id in self.subscriptions:
                ...
                self.logger.info("Push subscription removed", subscription_id=subscription_id)
    ...
    async def send_notification(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if not title:
                return web.json_response(
                    {"type": "error", "message": "Missing 'title' in request body", "code": 400},
                    status=400,
                )
            ...
            if subscription_id:
                ...
                # 发送到所有活跃订阅
                targets = [
                    sid for sid, sub in self.subscriptions.items() if sub.get("active", True)
                ]

            if not targets:
                return web.json_response(
                    {"type": "error", "message": "No active subscriptions found", "code": 404},
                    status=404,
                )
            ...
            for sid in targets:
                try:
                    ...
                    if self.pywebpush:
                        ...
                        # 模拟发送（记录日志）
                        self.logger.info(
                            "Mock push notification", subscription_id=sid, title=title, body=body
                        )
                    ...
                    self.logger.error(
                        "Failed to send notification", subscription_id=sid, error=str(e)
                    )
                    results.append({"subscription_id": sid, "success": False, "error": str(e)})
                    failure_count += 1
            ...
            return web.json_response(
                {"type": "error", "message": f"Failed to send notification: {str(e)}", "code": 500},
                status=500,
            )
    ...
    async def send_session_state_notification(
        self, session_id: str, state: str, message: str = None
    ) -> bool:
        """
        发送会话状态变更通知
        ...
        Returns:
            是否成功发送
        """
        titles = {"started": "会话已启动", "stopped": "会话已停止", "error": "会话错误"}

        payload = {
            "title": titles.get(state, "会话更新"),
            "body": message or f"会话 {session_id} 状态变更为 {state}",
            "data": {"type": "session_state", "session_id": session_id, "state": state},
        }

        return await self._send_to_all_subscriptions(payload)

    async def send_task_completion_notification(self, task_id: str, result: str) -> bool:
        """
        发送任务完成通知
        ...
        payload = {
            ...
            "data": {"type": "task_completion", "task_id": task_id, "result": result},
        }

        return await self._send_to_all_subscriptions(payload)

    async def send_error_notification(self, error_type: str, error_message: str) -> bool:
        """
        发送错误通知
        ...
        payload = {
            ...
            "body": error_message,
            "data": {"type": "error", "error_type": error_type},
        ...
        for sid, sub in self.subscriptions.items():
            ...
            try:
                if self.pywebpush:
                    ...
                    # 模拟发送
                    self.logger.info("Mock push notification", subscription_id=sid, payload=payload)

                success_count += 1

            except Exception as e:
                self.logger.error("Failed to send notification", subscription_id=sid, error=str(e))
