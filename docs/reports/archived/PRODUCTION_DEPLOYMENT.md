...
Type=oneshot
RemainAfterExit=yes
WorkingDirectory=/home/ai/zhibridge
ExecStart=/usr/local/bin/docker-compose -f /home/ai/zhibridge/docker-compose.prod.yml up -d
ExecStop=/usr/local/bin/docker-compose -f /home/ai/zhibridge/docker-compose.prod.yml down
TimeoutStartSec=0
...
添加以下行（每天凌晨 2 点备份）:
```cron
0 2 * * * /home/ai/zhibridge/scripts/backup.sh -t full -k 7
```
